...
| `POST` | `/api/pages` | Optional | Publish HTML → `{ url, id, expires_at }`. Accepts `application/json` (`{ html, title?, permanent? }`) or `multipart/form-data` (`file=@report.html`, `permanent?`). Guest pages expire in 30 min and ignore `permanent`. Authenticated pages expire per `RetentionPolicy` (default `CLEANUP_RETENTION_DAYS`), or never if `permanent: true`. |
