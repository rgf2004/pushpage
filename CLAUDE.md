...
| `POST` | `/api/pages` | Optional | Publish HTML (JSON) → `{ url, id }`. Guest pages expire in 30 min. |
| `POST` | `/api/pages/upload` | Optional | Publish HTML file (multipart) → `{ url, id }`. Title extracted from `<title>` tag, then filename. |
